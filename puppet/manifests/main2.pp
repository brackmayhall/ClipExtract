class extract-gg-env {
  ...
  $tomcat_url = "http://www-eu.apache.org/dist/tomcat/tomcat-8/v8.0.36/bin/apache-tomcat-8.0.36.tar.gz"
...
service { 'mongodb':
  ...
  exec { 'allow remote mongo connections':
    command => "/usr/bin/sudo sed -i 's/bind_ip = 127.0.0.1/bind_ip = 127.0.0.1,192.168.33.10/g' /etc/mongodb.conf",
    require => Package['mongodb'],
